...
toDo(){
    ...
    echo 'bash -c "$(curl -fsSL https://raw.githubusercontent.com/egmsystems/sh/main/pve.sh)"'
    echo bash: line 4: /root/function.sh: No such file or directory
    echo
    echo Apt-Cacher-NG
...
main() {
    ...
    echo ´bash -c "$(wget -qLO - https://raw.githubusercontent.com/MacRimi/ProxMenux/main/install_proxmenux.sh)"´

    local APT_CACHER="192.168.50.78"
    configure_apt_cacher $APT_CACHER 100
    upDateGradeRemoving
    rm /etc/apt/apt.conf.d/01proxy
    echo http_proxy="http://$APT_CACHER/"
    echo "time apt -y update"
    echo "time apt -y upgrade"
    echo "time apt -y autoremove"
    #toDo
}
time main